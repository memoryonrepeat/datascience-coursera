...
def update_state_score(tweet):
	tweet_obj=json.loads(tweet)
	if 'coordinates' in tweet_obj:		
		if tweet_obj['coordinates'] is not None:
			for state in state_bounding_box:
				if state_bounding_box[state][0]<=tweet_obj['coordinates']['coordinates'][1] and state_bounding_box[state][1]>=tweet_obj['coordinates']['coordinates'][1] and state_bounding_box[state][2]<=tweet_obj['coordinates']['coordinates'][0] and state_bounding_box[state][3]>=tweet_obj['coordinates']['coordinates'][0]:
					state_scores[state]+=calculate_sentiment(tweet)

def happiest_state(sent_file,tweet_file):
	build_sentiment_dict(sent_file)
	for line in tweet_file:
		update_state_score(line)
	# for state in state_scores:
	# 	print state +' '+str(state_scores[state])
	print state_abbr[max(state_scores, key=state_scores.get)]	
